...
3) Model 1 answer deepseek_answer
...
if st.button("Run tool"):
    ...
    # 1) Ask local models for ALL questions
    with st.spinner("the local models are thinking..."):
        ...
        for idx, (_, row) in enumerate(qa_df.iterrows(), start=1):
            ...
            # Update counter text
            counter_text.text(f"Processing question {idx} of {total_questions}...")

            # Call local models
            deepseek_resp = get_ollama_response(q, "deepseek-r1:7b")
            qwen_resp = get_ollama_response(q, "qwen3-vl:8b")

            deepseek_answers.append(deepseek_resp)
            qwen_answers.append(qwen_resp)

            # Update progress bar
            progress_bar.progress(idx / total_questions)

        # Add answers to DataFrame
        qa_df["deepseek_answer"] = deepseek_answers
    ...
    st.dataframe(
        qa_df[["id", "question", "ground_truth", "deepseek_answer", "qwen_answer"]].head()
    )
